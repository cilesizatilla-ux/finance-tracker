...
import { loginApi, register, googleAuth } from '../api/index.js'
...
export default function Login() {
  ...
  const handleGoogle = async (credentialResponse) => {
    ...
    try {
      ...
      const d = res.data?.data
      if (res.data?.error) { setError(res.data.error); return }
      if (!d?.access_token) { setError('Google login failed. Please try again.'); return }
        ...
        <div className="w-full max-w-sm">
          ...
          {googleClientId ? (
            ...
            <button
              type="button"
              disabled
              className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium mb-6 opacity-40 cursor-not-allowed"
            ...
            >
              ...
              <span className="text-xs" style={{ color: '#475569' }}>(not configured)</span>
            </button>
          )}
